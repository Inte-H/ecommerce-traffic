...
- **상태**: 측정 완료 — 졸업 조건 2/2 충족 (2026-08-17). 섹션 3(코틀린 기능 인덱스) 작성 중
...
`findStock()`으로 찾은 재고를 if 문에서 비교 후 decrease()로 차감하는데 차감은 원자적으로 처리해 판매와 재고 감소가 같지만 @Transactional이 있어도 READ COMMITTED 격리 레벨에서 SELECT는 lock이 잡히지 않아 다른 트랜잭션이 중간에 개입 가능함
...
select에서 lock이 걸리지 않아 판단과 차감 시점의 재고수가 다르므로 select for update를 도입해 적은 변화로 문제를 해결한다. 나중 phase에서 조건부 update나 낙관적 락과 차이를 본다
