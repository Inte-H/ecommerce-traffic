...
- **보고서 뼈대와 해석 질문**: 보고서의 `TODO(사용자)` 주석에는 질문과 봐야 할 표·열의 위치만 남긴다. 인과 사슬, 결론, 수치 해석을 미리 적으면 사용자는 받아쓰기만 하게 된다. 사용자가 해석에 대해 물으면 결론 대신 후보를 가르는 질문과 근거가 있는 위치를 답한다. 다음 phase 의 가설과 졸업 조건은 사용자가 먼저 쓴 뒤에 AI 가 검토한다.
